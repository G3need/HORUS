# ==============================================================================
# HORUS v10.1.py - Enhanced QR Decoding & Security Fixes
# ==============================================================================

import os, json, hashlib, random, threading, logging, datetime
import sqlite3
import numpy as np
import gradio as gr
import qrcode
...
def install_dependencies():
    """Install all required dependencies for HORUS v10.1"""
    dependencies = [
        "gradio>=4.0.0",
        "numpy",
        "reportlab",
    ...
    for dep in dependencies:
        try:
            import importlib
            importlib.import_module(dep.replace(">=", "").replace("[pil]", "").replace("-", "_"))
...
# ==============================================================================
# 🏛️ CONFIGURATION & DATA MODELS
# ==============================================================================

@dataclass
class HorusConfig:
    APP_NAME: str = "Horus Key Platinum v10.1"
    DB_NAME: str = "HorusDB.db"
    AI_MODEL: str = "gemini-1.5-flash"
    VERSION: str = "10.1.0-Gold (Enhanced QR)"
    PRICING: dict = {
        "visa_fee": 2000,
...
class TransportMode(Enum):
    ...
    BUS = "Bus"
    TAXI = "Taxi"
    RIDE_HAILING = "Online Ride-Hailing"
...
class HorusSecurity:
    ...
    @staticmethod
    def scan_face(image: np.ndarray) -> Optional[str]:
        """
        Enhanced biometric scanning with strict None handling
...
class QRDecoder:
    """
    NEW: QR Code Decoding class using pyzbar for enhanced QR functionality
    Supports decoding from webcam images and various QR formats
    """
    
    @staticmethod
    def decode_from_image(image: np.ndarray) -> List[str]:
        """
...
class VisaPolicy:
    ...
    @staticmethod
    def get_nationality_group(nationality: str) -> NationalityGroup:
...
class PriceCalculator:
    @staticmethod
    def calculate_ticket_price(base_price: float, nationality_group: NationalityGroup, visitor_type: VisitorType, quantity: int = 1) -> float:
        multiplier = 1.0
...
class HorusDB:
    ...
    def register_traveler(self, name, full_name, passport, nationality, nationality_group, expiry, bio_hash, occupation, purpose_of_travel, accommodation_address):
        ...
        self.cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (name, full_name, passport, nationality, nationality_group.value, expiry, bio_hash, occupation, purpose_of_travel, accommodation_address, datetime.datetime.now().isoformat()))
        self.conn.commit()
    ...
    def create_ticket(self, traveler_id, monument_name, visitor_type, quantity, price):
        ...
        self.cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (traveler_id, monument_name, visitor_type.value, quantity, price, datetime.datetime.now().isoformat()))
        self.conn.commit()
        return True
    
    def add_green_points(self, tid, points):
        """Add green points to traveler"""
        self.cursor.execute("UPDATE travelers SET green_points=green_points+? WHERE id=?", (points, tid))
        self.conn.commit()
        return True
    
    def claim_gift(self, tid):
        """Claim welcome gift - returns success status"""
        traveler = self.get_traveler(tid)
...
class GitAutopilot:
    @staticmethod
    def sync_codebase():
        """Sync codebase to GitHub repository"""
        try:
...
def get_rotated_key():
    """Get API key with rotation logic"""
...
def ask_ai(msg, history):
    """AI chat interface with enhanced error handling"""
    key = get_rotated_key()
    if not key: 
        return "⚠️ Configuration Error: Please grant access to 'GEMINI_KEYS' in the Colab tab, then try again."
    try:
...
# ==============================================================================
# 📱 UI (PLATINUM DASHBOARD) - DEEP DATA UPGRADE
# ==============================================================================
...
def ui_activate_wallet(card_number):
    """Activate wallet with enhanced validation"""
    global current_user
...
def process_qr_payment(qr_string):
    """Process QR payment with enhanced validation"""
    global current_user
...
def ui_demo_login():
    ...
    uid = db.register_traveler(name, full_name, "D999999", "USA", NationalityGroup.FOREIGN, "2030-01-01", "BIO-DEMO-KEY", "Diplomat", "Official Visit", "Cairo Marriott Hotel")
...
with gr.Blocks(css=css, title="Horus Key Platinum v10.1") as demo:
    # LOGO EMBEDDING
